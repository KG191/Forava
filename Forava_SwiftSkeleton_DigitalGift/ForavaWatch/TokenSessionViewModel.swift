...
final class TokenSessionViewModel: ObservableObject {
    ...
    init() {
        WatchSessionManager_Watch.shared.activate()
    }
    ...
    func sendGiftRequest() {
        guard let t = activeToken else { return }
        WatchSessionManager_Watch.shared.sendGiftRequest(
            token: t, amountMinor: amountMinor, currency: Constants.defaultCurrency, kind: kind
        ) { success in
