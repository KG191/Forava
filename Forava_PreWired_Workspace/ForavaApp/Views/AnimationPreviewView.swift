...
struct SparkleEffectView: View {
    ...
    var body: some View {
        ZStack {
            ForEach(0..<Int(density * 10), id: \.self) { _ in
                Circle()
                    .fill(.white)
...
#Preview {
    AnimationPreviewView(
        ...
        frames: Array(0..<12).map { frameIndex in
                OptimizedAnimationFrame(
                    frameNumber: frameIndex,
                    timestamp: TimeInterval(frameIndex) * 0.25,
                    imageData: Data(),
                    compressionLevel: .medium,
            effects: [.glow(intensity: Float(sin(Double(frameIndex) * 0.5)) * 0.5 + 0.5, color: .orange, radius: 10.0)]
                )
            },
            duration: 3.0,
