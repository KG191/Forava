...
class MockAIRakhiService: ObservableObject {
...
struct PreviewStep: View {
    ...
    let aiService: MockAIRakhiService
    @State private var showingPaymentSettings = false

    init(designSpec: RakhiDesignSpec, aiService: MockAIRakhiService = MockAIRakhiService()) {
...
struct DesignPreviewCard: View {
    ...
    private func calculateCulturalScore() -> Double {
        // Mock cultural score calculation
        return Double(designSpec.elements.count) * 0.15 + 0.3
    }
}
