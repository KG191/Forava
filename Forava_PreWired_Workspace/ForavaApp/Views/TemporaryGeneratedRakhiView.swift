...
// MARK: - Temporary Generated Cultural Gift View (Phase 5: Dynamic Terminology)
struct TemporaryGeneratedRakhiView: View {
    ...
    @StateObject private var fixedSharingService = FixedSocialSharingService.shared
            ...
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    VStack(spacing: 16) {
                        ...
                        VStack(spacing: 8) {
                            Text(terminologyService.giftReadyMessage)
...
// MARK: - Send Options Implementation Extension
extension TemporaryGeneratedRakhiView {
    private func sendToAppleWatch() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            ...
            let payURL = AppConfig.universalPayURL(amount: suggestedAmount, desc: "Rakhi Blessing Gift",
                                                   rakhiId: generatedRakhi.id.uuidString, sender: "Forava Creator")
            print("🔗 Pay URL: \(payURL?.absoluteString ?? "nil")")
    ...
    // PHASE 2 FIXED: WhatsApp sharing with proper error handling and freeze prevention
    private func sendViaWhatsApp() {
        print("🔄 [FIXED] Starting WhatsApp sharing to \(recipient.name)")

        // Get the current view controller safely
        guard let viewController = getCurrentViewController() else {
            print("❌ Could not get current view controller")
            return
        }

        // Use dynamic terminology for message
        let culturalMessage = "🎊 I've created a beautiful \(terminologyService.digitalGiftTerm) for you! Made with love using Forava ❤️"

        Task {
            await fixedSharingService.shareToWhatsApp(
            ...
            // Only dismiss if sharing was successful or handled
            if !fixedSharingService.isSharing {
                onDismiss()
            }
        }
    }

    // PHASE 2 FIXED: Safe view controller retrieval
