...
class AdvancedCustomizationService: ObservableObject {
    ...
    func startCustomizationSession(from rakhi: GeneratedRakhi) -> CustomizationSession {
        ...
        isCustomizing = true
        generateRealTimePreview(for: session)
    ...
    private func generateRealTimePreview(for session: CustomizationSession) async throws {
...
extension AdvancedCustomizationService {
    func calculateOptimalPatternPosition(
    ...
    func updateCustomizationParameter(
    ...
    func scaleCustomizationIntensity(
        _ customization: AnyCustomization,
        by intensity: Float
    ) -> AnyCustomization {
    ...
    func calculateCulturalCompatibility(for session: CustomizationSession) -> Float {
        ...
        return culturalCustomizations.isEmpty ? 0.5 : Float(culturalCustomizations.map { $0.intensity }.reduce(0, +) / Double(culturalCustomizations.count))
    }

    func generatePresetTags(for session: CustomizationSession) -> [String] {
        var tags: [String] = []
...
extension AdvancedCustomizationService {
    ...
    func saveCustomizationPreset(_ preset: CustomizationPreset) {
    ...
    func loadCustomizationPresets() -> [CustomizationPreset] {
...
enum AnyCustomization: Identifiable {
    ...
    var id: UUID {
        switch self {
        case .pattern(let pattern): return pattern.id
        case .texture(let texture): return texture.id
        case .material(let material): return material.id
        case .colorGrading(let colorGrading): return colorGrading.id
        case .geometry(let geometry): return geometry.id
        case .cultural(let cultural): return cultural.id
        }
    }
}
