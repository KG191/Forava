...
class SocialSharingService: NSObject, ObservableObject {
    ...
    func shareViaEmail(
        ...
        let emailSubject = subject ?? "🎊 A Special Rakhi Created Just for You!"
        let emailBody = createCulturalEmailBody(shareContent: shareContent, rakhi: rakhi)
    ...
    private func createRakhiDeepLink(_ rakhi: GeneratedRakhi) -> URL {
        // Phase 3: Integration with Cultural Payment Page Service
        if let culturalGift = convertToCultural(rakhi) {
            return CulturalPaymentPageService.shared.generateCulturalPaymentURL(
                for: culturalGift,
                senderName: getUserDisplayName(),
                recipientName: "Recipient", // Would be dynamic in real implementation
                selectedOccasion: getCurrentOccasion(from: rakhi)
            )
        }

        // Fallback to original deep link for legacy support
    ...
    // Phase 3: Helper methods for cultural integration
    private func convertToCultural(_ rakhi: GeneratedRakhi) -> CulturalGeneratedArtwork? {
        // Convert legacy GeneratedRakhi to CulturalGeneratedArtwork
        // This would be a proper conversion in production
        return nil // Placeholder - would implement proper conversion
    }

    private func getUserDisplayName() -> String {
        // Get user's display name from settings or profile
        return UserDefaults.standard.string(forKey: "user_display_name") ?? "A Friend"
    }

    private func getCurrentOccasion(from rakhi: GeneratedRakhi) -> String {
        // Determine occasion from rakhi or current date
        if let currentContext = CulturalConfiguration.shared.currentCulturalContext {
            switch currentContext {
            case "hindu_festivals":
                return determineFestival() ?? "Raksha Bandhan"
            case "chinese_traditional":
                return "Chinese New Year"
            case "christian_traditional":
                return "Christmas"
            case "universal_celebrations":
                return "Birthday"
            default:
    ...
    private func determineFestival() -> String? {
        let calendar = Calendar.current
        let now = Date()
        let month = calendar.component(.month, from: now)
        let day = calendar.component(.day, from: now)

        // Simple festival detection based on approximate dates
        if month == 10 || month == 11 { // Diwali season (October-November)
            return "Diwali"
        } else if month == 3 { // Holi season (March)
            return "Holi"
        } else if month == 8 && day >= 10 && day <= 25 { // Raksha Bandhan
            return "Raksha Bandhan"
        }
    ...
    private func createCulturalEmailBody(shareContent: ShareContent, rakhi: GeneratedRakhi) -> String {
        let occasion = getCurrentOccasion(from: rakhi)
        let occasionEmoji = getOccasionEmoji(for: occasion)

        return """
        <html>
        <body style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                <h1 style="color: #FF6B35; text-align: center;">\(occasionEmoji) A Special \(occasion) Greeting for You!</h1>

                <p style="font-size: 16px; line-height: 1.6;">
                    I've created a beautiful cultural greeting using AI technology while honoring our cherished traditions.
                    This unique design carries the essence of our cultural values and the warmth of celebration.
                </p>

                <div style="text-align: center; margin: 30px 0;">
                    <img src="data:image/png;base64,{image_data}" alt="AI Generated Cultural Greeting" style="max-width: 400px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
                </div>
                ...
                <div style="text-align: center; margin: 30px 0;">
                    <a href="\(shareContent.rakhiURL.absoluteString)" style="background: #FF6B35; color: white; padding: 12px 24px; text-decoration: none; border-radius: 25px; font-weight: bold;">
                        View Your Cultural Gift
                    </a>
                </div>

                <p style="font-size: 14px; color: #666; text-align: center; margin-top: 40px;">
                    Made with ❤️ for \(occasion) - Where tradition meets technology
                </p>
            </div>
        </body>
        </html>
...
// MARK: - Analytics & Metadata Extension
extension SocialSharingService {
    func generateSharingMetadata(for rakhi: GeneratedRakhi) -> SharingMetadata {
    ...
    func getUserSharePreferences() -> SharePreferences {
    ...
    func recordShareActivity(rakhi: GeneratedRakhi, method: ShareMethod, result: ShareResult) {
...
extension SocialSharingService {
    func truncateForInstagram(_ text: String) -> String {
        // Instagram caption limit
        let maxLength = 2200
        return text.count > maxLength ? String(text.prefix(maxLength - 3)) + "..." : text
    }

    func optimizeForInstagram(_ image: UIImage) -> UIImage {
        // Optimize image for Instagram (1080x1080)
        return image // Placeholder - would implement actual optimization
    }
}

// MARK: - Platform-Specific Sharing Extension
extension SocialSharingService {
    func shareToWhatsApp(_ rakhiId: UUID, recipients: [Contact], content: PlatformContent) async throws -> ShareResult {
        // Implementation for WhatsApp sharing
        return ShareResult(success: true, platform: .whatsapp, timestamp: Date())
    }

    func shareToInstagramPlatform(content: PlatformContent) async throws -> ShareResult {
        // Implementation for Instagram sharing
        return ShareResult(success: true, platform: .instagram, timestamp: Date())
    }

    func shareViaEmailPlatform(content: PlatformContent, recipients: [Contact]) async throws -> ShareResult {
        // Implementation for email sharing
        return ShareResult(success: true, platform: .email, timestamp: Date())
    }

    func shareToInstagramStory(_ content: ShareContent) async throws -> ShareResult {
    ...
    func shareToInstagramPost(_ content: ShareContent) async throws -> ShareResult {
...
// MARK: - Contact Model removed - using main Contact model from Models/Contact.swift
