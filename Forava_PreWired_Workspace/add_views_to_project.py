...
# Files to add - focusing on the missing Views that are causing compilation errors
new_files = [
    ("ForavaApp/Views/CulturalDesignStudioView.swift", "CulturalDesignStudioView.swift"),
    ("ForavaApp/Views/CulturalGiftPreviewView.swift", "CulturalGiftPreviewView.swift"),
    ("ForavaApp/Views/ContactSelectionView.swift", "ContactSelectionView.swift"),
    ("ForavaApp/Views/Settings/SubscriptionManagementView.swift", "SubscriptionManagementView.swift"),
]
...
# Find the ForavaApp group and add new files to it
# Look for the ForavaApp group children section
foravaapp_group_pattern = r'(A1FE /\* ForavaApp \*/ = {[^}]*children = \()[^)]*(\);)'
foravaapp_match = re.search(foravaapp_group_pattern, content, re.DOTALL)

if foravaapp_match:
    existing_children = foravaapp_match.group(0)
...
sources_build_phase_pattern = r'(A1FA /\* Sources \*/ = {[^}]*files = \()[^)]*(\);)'
...
print("Successfully added Views files to Xcode project!")
print("Files added:")
for file_path, file_name in new_files:
    print(f"  - {file_name}")
