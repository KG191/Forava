...
class AdvancedAnimationService: ObservableObject {
    ...
    private func analyzeImageForAnimation(_ imageResult: AIImageResult) async throws -> ImageAnimationAnalysis {
        ...
        return ImageAnimationAnalysis(
            dominantColors: [.red, .gold, .orange],
            detectedElements: [
    ...
    private func generateFrameEffects(
        ...
        case .sparkle_effect:
            return [
                FrameEffect.sparkle(
                    ...
                    anchorPoints: analysis.detectedElements.map { $0.region.center }
                )
            ]
        ...
        case .custom(let effects, _):
            return effects.map { effect in
                effect.generateFrameEffect(progress: progress, analysis: analysis)
            }
        }
    }
    ...
    private func loadAnimationTemplates() {
        availableAnimations = [
            ...
            AnimationTemplate(
                ...
                animationType: .cultural_blessing,
                culturalAppropriate: true,
                batteryFriendly: false,
...
extension AnimationType {
    ...
    var defaultDuration: TimeInterval {
        switch self {
        case .subtle_glow: return 3.0
        case .sparkle_effect: return 2.5
        case .gentle_pulse: return 4.0
        case .thread_shimmer: return 2.0
        case .cultural_blessing: return 5.0
    ...
    var batteryImpactPerFrame: Float {
        switch self {
        case .subtle_glow: return 0.02
        case .sparkle_effect: return 0.05
        case .gentle_pulse: return 0.01
        case .thread_shimmer: return 0.03
        case .cultural_blessing: return 0.08
        case .custom(let effects, _):
            return effects.reduce(0) { $0 + $1.batteryImpact }
        }
    }
}
...
struct AnimationEffect {
    ...
    func generateFrameEffect(progress: Float, analysis: ImageAnimationAnalysis) -> FrameEffect {
        switch type {
        case .glow:
            return .glow(intensity: intensity, color: .orange, radius: 10.0)
        case .pulse:
        ...
        case .blessing:
            return .aura(color: .gold, intensity: intensity, pulseDuration: 2.0)
        }
    }
}
