...
struct SharingOptionsGrid: View {
    ...
    private let sharingOptions: [(ShareMethod, String, String, Color)] = [
        (.whatsapp, "WhatsApp", "logo.whatsapp", .green),
        (.instagram, "Instagram", "camera", .pink),
        (.messages, "Messages", "message.fill", .blue),
        (.email, "Email", "envelope.fill", .gray),
        (.facebook, "Facebook", "f.circle.fill", .blue),
        (.twitter, "Twitter", "bird.fill", .cyan)
    ]
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 3), spacing: 16) {
                ForEach(Array(sharingOptions.enumerated()), id: \.offset) { index, option in
                    SharingOptionButton(
                        method: option.0,
                        title: option.1,
                        icon: option.2,
