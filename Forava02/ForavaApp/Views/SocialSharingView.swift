    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showingContactPicker) {
                SharingContactPickerView(selectedContacts: $selectedContacts)
            }
        ...
        VStack(spacing: 16) {
            ...
            VStack(spacing: 8) {
                ...
                HStack(spacing: 16) {
                    DetailBadge(
                        icon: "star.fill",
                        text: "Cultural Score: \(String(format: "%.1f", rakhi.culturalScore))",
                        color: .green
                    )
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 3), spacing: 16) {
                ForEach(sharingOptions, id: \.method) { option in
                    SharingOptionButton(
    ...
    var body: some View {
        LazyVStack(spacing: 8) {
            ForEach(selectedContacts) { contact in
                SharingContactRow(contact: contact) {
...
struct SharingContactPickerView: View {
    @Binding var selectedContacts: [Contact]
    @Environment(\.dismiss) private var dismiss
