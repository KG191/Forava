...
struct RoshHashanahStyleSelectionView: View {
    @Binding var selectedTheme: RoshHashanahTheme?
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                // Header Section
                VStack(spacing: 12) {
                    Text("Choose Your Rosh Hashanah Style")
                        .font(.system(.title2, design: .rounded).weight(.bold))
                        .foregroundStyle(.primary)

                    Text("Select the style that best reflects your High Holy Days celebration")
