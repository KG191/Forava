...
struct EidAlAdhaCreateSummaryView: View {
    let selectedTheme: EidAlAdhaTheme?
    let selectedElements: [EidAlAdhaElement]
    let selectedColorPalette: EidAlAdhaColorPalette?
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                // Header Section
                VStack(spacing: 12) {
                    Text("Create Your EidAlAdha Gift")
    ...
    private func generationPreview() -> some View {
        VStack(spacing: 12) {
            ...
            Text("Your personalized EidAlAdha gift will include both iPhone and Apple Watch formats")
...
#Preview {
    EidAlAdhaCreateSummaryView(
        ...
        selectedElements: [EidAlAdhaElement.allElements[0]],
        selectedColorPalette: EidAlAdhaColorPalette.allPalettes[0],
        finalMessage: "Wishing you a blessed Eid al-Adha filled with joy and peace",
        isReadyToGenerate: true,
        culturalColor: Color(hex: "#2E7D32"),
        onGenerate: {}
    )
}
