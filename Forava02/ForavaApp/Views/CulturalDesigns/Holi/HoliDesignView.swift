...
struct HoliDesignView: View, CulturalDesignViewProtocol {
    // MARK: - Protocol Requirements
    typealias CulturalTheme = HoliTheme
    typealias CulturalElement = HoliElement
    typealias CulturalColorPalette = HoliColorPalette
    typealias CulturalPersonalTouch = HoliPersonalTouch

    // MARK: - Content View Types
    typealias StyleContent = HoliStyleSelectionView
    typealias ElementsContent = HoliElementsSelectionView
    typealias ColorContent = HoliColorPaletteView
    typealias TouchContent = HoliPersonalTouchView
    typealias CreateContent = HoliCreateSummaryView
    typealias CheckContent = HoliCheckImageView
    typealias SendContent = HoliSendShareView
    typealias ConnectContent = HoliConnectView
    ...
    @State var selectedTheme: HoliTheme?
    @State var selectedElements: [HoliElement] = []
    @State var selectedColorPalette: HoliColorPalette?
    @State var selectedMessage: HoliPersonalTouch?
    ...
    @StateObject private var holiAI = HoliAIService.shared
...
extension HoliDesignView {
    ...
    private func generateHoliGift() {
        print("🎯 Generate Holi Gift button tapped")
...
print("=" + String(repeating: "=", count: 79))
        ...
        print("✅ Starting Holi generation...")
        isGenerating = true
        currentTab = .check
        ...
        Task {
            do {
                ...
                let iPhoneResult = try await holiAI.generateHoliGift(
                    theme: selectedTheme!,
                ...
                let watchResult = try await holiAI.generateHoliGift(
                    theme: selectedTheme!,
                    element: selectedElement,
                    colorPalette: selectedColorPalette!,
                    message: "", // No text in AI - will overlay natively
    ...
    private func regenerateWithCreditCheck() {
        ...
        if hasGeneratedOnce {
            ...
            if paymentService.hasRegenerationCredits() {
                ...
                let success = paymentService.useRegenerationCredit(for: "Holi")
                if success {
                    print("✅ Credit used for Holi regeneration")
                    generateHoliGift()
                } else {
