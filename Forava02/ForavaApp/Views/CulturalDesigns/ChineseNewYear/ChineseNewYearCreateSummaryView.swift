...
struct ChineseNewYearCreateSummaryView: View {
    let selectedTheme: ChineseNewYearTheme?
    let selectedElements: [ChineseNewYearElement]
    let selectedColorPalette: ChineseNewYearColorPalette?
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                // Header Section
                VStack(spacing: 12) {
                    Text("Create Your Chinese New Year Gift")
    ...
    private func generationPreview() -> some View {
        VStack(spacing: 12) {
            ...
            Text("Your personalized Chinese New Year gift will include both iPhone and Apple Watch formats")
    ...
    private var elementsDetailDescription: String {
        ...
        return selectedElements.map { "\($0.name) (\($0.chineseName))" }.joined(separator: ", ")
    }
...
#Preview {
    ChineseNewYearCreateSummaryView(
        ...
        selectedElements: [ChineseNewYearElement.allElements[0]],
        selectedColorPalette: ChineseNewYearColorPalette.allPalettes[0],
        finalMessage: "恭喜发财 - Wishing you prosperity",
