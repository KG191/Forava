...
struct VesakDayStyleSelectionView: View {
    @Binding var selectedTheme: VesakDayTheme?
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                // Header Section
                VStack(spacing: 12) {
                    Text("Choose Your Vesak Day Style")
                        .font(.system(.title2, design: .rounded).weight(.bold))
                        .foregroundStyle(.primary)

                    Text("Select the style that best reflects your Buddhist celebration")
...
#Preview {
    VesakDayStyleSelectionView(
        ...
        culturalColor: Color(hex: "#F99600") // Saffron
    )
}
