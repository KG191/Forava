...
struct VesakDayDesignView: View, CulturalDesignViewProtocol {
    // MARK: - Protocol Requirements
    typealias CulturalTheme = VesakDayTheme
    typealias CulturalElement = VesakDayElement
    typealias CulturalColorPalette = VesakDayColorPalette
    typealias CulturalPersonalTouch = VesakDayPersonalTouch

    // MARK: - Content View Types
    typealias StyleContent = VesakDayStyleSelectionView
    typealias ElementsContent = VesakDayElementsSelectionView
    typealias ColorContent = VesakDayColorPaletteView
    typealias TouchContent = VesakDayPersonalTouchView
    ...
    @State var selectedTheme: VesakDayTheme?
    @State var selectedElements: [VesakDayElement] = []
    @State var selectedColorPalette: VesakDayColorPalette?
    @State var selectedMessage: VesakDayPersonalTouch?
    ...
    @StateObject private var vesakDayAI = VesakDayAIService.shared
    ...
    var body: some View {
        ...
        .navigationTitle("Vesak Day Design")
...
extension VesakDayDesignView {
    @ViewBuilder func styleContent() -> StyleContent {
        VesakDayStyleSelectionView(
    ...
    @ViewBuilder func createContent() -> CreateContent {
        VesakDayCreateSummaryView(
            ...
            culturalColor: culturalColor,
            onGenerate: {
                generateVesakDayGift()
            }
        )
    }

    @ViewBuilder func checkContent() -> CheckContent {
        VesakDayCheckImageView(
            generatedImages: generatedImages,
            personalMessage: finalMessage,
    ...
    private func generateVesakDayGift() {
        print("🎯 Generate Vesak Day Gift button tapped")
...
print("=" + String(repeating: "=", count: 79))
        ...
        print("✅ Starting Vesak Day generation...")
        isGenerating = true
        currentTab = .check
        ...
        Task {
            do {
                ...
                let iPhoneResult = try await vesakDayAI.generateVesakDayGift(
                    theme: selectedTheme!,
                ...
                let watchResult = try await vesakDayAI.generateVesakDayGift(
                    theme: selectedTheme!,
                    element: selectedElement,
                    colorPalette: selectedColorPalette!,
                    message: "", // No text in AI - will overlay natively
    ...
    private func regenerateWithCreditCheck() {
        ...
        if hasGeneratedOnce {
            ...
            if paymentService.hasRegenerationCredits() {
                ...
                let success = paymentService.useRegenerationCredit(for: "Vesak Day")
                if success {
                    print("✅ Credit used for Vesak Day regeneration")
                    generateVesakDayGift()
                } else {
