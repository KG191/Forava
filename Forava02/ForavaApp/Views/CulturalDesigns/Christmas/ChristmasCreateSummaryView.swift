...
struct ChristmasCreateSummaryView: View {
    let selectedTheme: ChristmasTheme?
    let selectedElements: [ChristmasElement]
    let selectedColorPalette: ChristmasColorPalette?
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                // Header Section
                VStack(spacing: 12) {
                    Text("Create Your Christmas Gift")
    ...
    private func generationPreview() -> some View {
        VStack(spacing: 12) {
            ...
            Text("Your personalized Christmas gift will include both iPhone and Apple Watch formats")
