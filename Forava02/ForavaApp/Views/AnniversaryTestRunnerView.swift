...
struct AnniversaryTestRunnerView: View {
    ...
    enum ConfigOption: String, CaseIterable {
        ...
        var description: String {
            switch self {
            case .quick:
                return "Runs critical baseline tests only (~30 tests, ~2 min)"
            case .standard:
                return "Runs all 240 tests with placeholder scoring (~8 min)"
            case .full:
                return "Runs all 240 tests with Vision API analysis (~12 min)"
            }
        }
    }
