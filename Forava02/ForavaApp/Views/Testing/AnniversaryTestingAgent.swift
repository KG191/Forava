...
class AnniversaryTestingAgent: ObservableObject {
    ...
    struct TestingConfiguration {
        ...
        // NOTE: 'full' configuration is currently NOT USED in the UI
        // Vision API/image analysis is not implemented - only placeholder scoring exists
        // Keep this configuration for future Vision API integration
        static let full = TestingConfiguration(
            enableImageAnalysis: true,  // TODO: Implement actual Vision API integration
            saveGeneratedImages: true,
    ...
    func saveReportToFile(report: TestReport, filename: String = "test_report.txt") throws {
        let fileManager = FileManager.default
        let documentsPath = fileManager.urls(for: .documentDirectory, in: .userDomainMask)[0]
        ...
        try reportText.write(to: reportPath, atomically: true, encoding: .utf8)
